...
//push from the center###############################################################
	  ...
	  {
		if (enemy == self) // enemy must not be vortex grenade
			continue;

		if (!enemy->client) // enemy must be a client
			continue;

		if (enemy == self->parent) // enemy must not be the player who fired the vortex grenade 
			continue;

		if (!enemy->takedamage) // enemy must be able to take damage
			continue;

		VectorCopy(enemy->r.currentOrigin, start); // put enemy position in start
		...
		VectorNormalize(dir); 
		VectorScale(dir,PUSH_FORCE, enemy->client->ps.velocity); //  scale directional vector by 200 and add to the targets velocity
		VectorCopy(dir, enemy->movedir); // make targets move direction = to directional vector.
	  }
//#############################################################################
...
//suck to center###############################################################
      ...
      if( enemy->client && enemy->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
	  {
		...
		VectorSubtract(end, start, dir); // subtract start from end to get directional vector
		VectorNormalize(dir); 
		VectorScale(dir,200, enemy->client->ps.velocity); //  scale directional vector by 200 and add to the targets velocity
		VectorCopy(dir, enemy->movedir); // make targets move direction = to directional vector.
	  }
//#############################################################################

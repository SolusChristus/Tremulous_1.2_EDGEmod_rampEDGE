...
g_admin_cmd_t g_admin_cmds[ ] =
  {
   ...
   {"randombps", G_admin_randombps, qfalse, "randombps",
     "re-randomize this match's build points",
     ""
   },
   ...
   {"setbps", G_admin_setbps, qfalse, "setbps",
     "set both teams build points",
     "[^3new number of build points^7]"
   },
...
qboolean G_admin_setbps( gentity_t *ent )
{
  char newPoints[ 5 ];
  int newBuildPoints = 150;
   
  if( trap_Argc() < 1 )
  {
    ADMP( "^3setbps: ^7usage: setbps [new number of build points]\n" );
    return qfalse;
  }

  trap_Argv( 1, newPoints, sizeof( newPoints ) );
  newBuildPoints = atoi( newPoints );
  trap_Cvar_Set( "g_alienBuildPoints", va( "%d", newBuildPoints ) );
  trap_Cvar_Set( "g_humanBuildPoints", va( "%d", newBuildPoints ) );
  return qtrue;
}

qboolean G_admin_randombps( gentity_t *ent )
{
  int buildPointModifier = 0;
  int newHumanBuildPoints = 0;
  int newAlienBuildPoints = 0;
  buildPointModifier = rand() % 700;
  
  ADMP( "^7Build Points randomized!\n" );

  newHumanBuildPoints = 150 + buildPointModifier;
  newAlienBuildPoints = 200 + buildPointModifier;

  trap_Cvar_Set( "g_alienBuildPoints", va( "%d", newAlienBuildPoints ) );
  trap_Cvar_Set( "g_humanBuildPoints", va( "%d", newHumanBuildPoints ) );
  return qtrue;
}
